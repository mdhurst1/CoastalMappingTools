...
ModernPath = WorkingPath / "MHWS_Lines" / ("Cullipool_Baseline.shp")
LiDARPath = WorkingPath / "MHWS_Lines" / ("Cullipool_Baseline.shp")
SoftPath = WorkingPath / "MHWS_Lines" / ("Cullipool_MHWS_1995.shp")
OldPath = WorkingPath / "MHWS_Lines" / ("Cullipool_MHWS_1890.shp")
QuiteOldPath = WorkingPath / "MHWS_Lines" / ("Cullipool_MHWS_1970.shp")

# get bathymetry path
BathyPath = WorkingPath / "Bathymetry" / ("Cullipool_Bathy.shp")
...
for Scenario, Percentile in zip(Scenarios, Percentiles):
    ...
    # write smoothed coast/bathy to file
    #ThisCoast.WriteCoastShp(str(OutputPath / (ProjectName + "_Smoothed_Baseline.shp")))
    #ThisCoast.WriteFutureShorelinesShp(str(OutputPath / (ProjectName + "_Future.shp")),Smooth=True)
