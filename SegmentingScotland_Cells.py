...
For segmenting beaches to count assets within
...
OutputPath = WorkingPath/"CoastalZoneClassification"

# set the transect spacing (in m)
TransectSpacing = 1000.
SmoothingWindowSize = 51

# read shore and intertidal shps as gdfs
ModernGDF = gp.read_file(str(WorkingPath / "MHWS_Lines" / "OS_strategy_coast250.shp"))
IntertidalGDF = gp.read_file(str(WorkingPath / "MHWS_Lines" / "OS_strategy_coast250_1km_buff.shp"))
...
for index, Row in Cells.iterrows():
    ...
    try:
        Intertidal.to_file(WorkingPath / "MHWS_Lines" / (RowName + "_1kmBuffer.shp"))
    except:
    ...
    if not CellCoast.BuiltTransects:
        ...
        CellCoast.IntersectTransectsWithIntertidal(str(WorkingPath / "MHWS_Lines" / (RowName + "_1kmBuffer.shp")))
