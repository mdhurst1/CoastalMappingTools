...
for Scenario, Percentile in zip(Scenarios, Percentiles):
    ...
    for index, Row in Cells.iterrows():
        ...
        OpenCoastFilename = OpenPath / (RowName+"_Change.pydata")
        try:
        ...
        OpenCoastFilename = OpenPath / (RowName+"_Change.pydata")
        try:
            InnerCoast = pickle.load( open( OpenCoastFilename, "rb" ) )
            InnerCoastFlag = True
        ...
        for i, Decade in enumerate(Decades):
            ...
            if OpenCoastFlag:
                ...
                OpenCoast.WriteErosionProximityShp(str(OpenPath / (RowName + "_Influence_" + str(Decade) + ".shp")), Year=Decade, Distance = 10.)
                OpenCoast.WriteErosionProximityShp(str(OpenPath / (RowName + "_Vicinity_" + str(Decade) + ".shp")), Year=Decade, Distance = 60.)
            ...
            if InnerCoastFlag:
                ...
                InnerCoast.WriteErosionProximityShp(str(InnerPath / (RowName + "_Influence_" + str(Decade) + ".shp")), Year=Decade, Distance = 10.)
                InnerCoast.WriteErosionProximityShp(str(InnerPath / (RowName + "_Vicinity_" + str(Decade) + ".shp")), Year=Decade, Distance = 60.)
